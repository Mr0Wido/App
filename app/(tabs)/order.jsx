...
const Order = () => {
  ...
  useEffect(() => {
    const calculatedTotalPrice = () => {
      ...
      const total = orders.reduce((accumulator, item) => accumulator + (item.amaount * item.price), 0);
      setsumOrder(total);
    }
            ...
            renderItem={({ item }) => (
              <View className="flex-row p-4 border-b border-gray-300">
                ...
                <View className="items-center justify-end">
                  ...
                  <Text className=" text-red-600 text-base font-pregular">{item.amaount * item.price} TL</Text>
                </View>
              </View>
            )}
          />
          <View className="items-center justify-center mt-7">
            <Text className="text-red-600 text-base font-pregular">Sipariş Tutarı: {sumOrder} TL</Text>
            <CustomButton 
